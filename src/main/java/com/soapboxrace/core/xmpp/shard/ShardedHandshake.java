...
public class ShardedHandshake implements IHandshake
{
    ...
    public ShardedHandshake()
    {
        ...
        openFireTalk = new ShardedOpenFireTalk(socketClient.getSocket());
        TlsWrapper.wrapXmppTalk(openFireTalk);
        
        String resource = String.format("EA-Chat_%s", PARAMETER_BO.isShardingMaster() ? "master" : PARAMETER_BO.getShardId());
